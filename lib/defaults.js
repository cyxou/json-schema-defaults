(function(root, factory) {
 ...
 'use strict';
   ...
   * @return {Object}
   */
  var defaults = function(schema, definitions, opts) {
    ...
    if (typeof schema['default'] !== 'undefined') {
      ...
      return defaults(mergedItem, definitions, opts);

    } else if (typeof schema.$ref !== 'undefined') {

      var reference = getLocalRef(schema.$ref, definitions);
      return defaults(reference, definitions, opts);
      ...
      for (var key in schema.properties) {
        if (schema.properties.hasOwnProperty(key)) {
          schema.properties[key] = defaults(schema.properties[key], definitions, opts);
      ...
      if (!opts.explicitOnly) {
        ...
        if (schema.items.constructor === Array) {
          var values = schema.items.map(function (item) {
            return defaults(item, definitions, opts);
          });
        ...
        // object-typed arrays
        var value = defaults(schema.items, definitions, opts);
   ...
   * @param {Object} opts
   * @prperty {string} opts.explicitOnly Set to true to generate default values
   *                                     only if the `default` property is specified
   *                                     in json schema.
   * @return {Object}
   */
  return function (schema, definitions, opts) {
    opts = opts || {};
    opts.explicitOnly = opts.explicitOnly || false;
    ...
    return defaults(cloneJSON(schema), definitions, opts);
  };
